...
templates:
	explicit specialization: or full specialization
		...
		why we need it?
			1. if the template we use won't be suitable for some template arguments, so we are providing another template
				to compiler to use in such cases
...
void func(int x)
{
	cout << "explicit specialization called for func(int) \n";
}
